...
#
# Arguments:    a0 contains the height of the figure
#               a1 contsins the width of the figure
#               a2 contains the address of the figure's object
#

block_make:
        addi    $sp, $sp,-FRAMESIZE_8           # Move stack pointer
        sw      $ra, -4+FRAMESIZE_8($sp)        # store return adr. on stack

        la      $t1, block_vtbl                 # get block's vtable pointer
        sw      $t1, 0($a2)                     # put block's vtable pointer
                                                # into this fig
        jal figure_make

        lw      $ra, -4+FRAMESIZE_8($sp)        # get ra off of stack 
                                                # and restore
...
block_area:
        addi    $sp, $sp, -FRAMESIZE_8          # move stack pointer
        sw      $ra, -4+FRAMESIZE_8($sp)        # store ra on stack
        
        lw      $t0, 4($a0)                     # get the block height
        lw      $t1, 8($a0)                     # get the block width
        
        mul     $t2, $t0, $t1                   # area = length * width

        add     $v0, $t2, $zero                 # put answer into v0

        lw      $ra, -4+FRAMESIZE_8($sp)        # load ra from stack
        addi    $sp, $sp, FRAMESIZE_8           # move stack pointer back
        jr      $ra
...
#
# Description:  Compute the perimeter of a block figure
#
# Arguments:    a0 contains the address of the block figure
#
...
block_perimeter:
        ...
        lw      $t0, 4($a0)                     # get height of block
        lw      $t1, 8($a0)                     # get width of block

        add     $t2, $t0, $t0                   # get sum of horizontal sides
        add     $t3, $t1, $t1                   # get sum of vertical sides
        add     $t4, $t2, $t3                   # get total sum of sides

        add     $v0, $t4, $zero                 # put sum into v0
...
#
        li      $v0, PRINT_INT          # print number
        lw      $a0, 4($s0)             # get width
        syscall
...
#
        ...
        add     $a0, $s0, $zero         # a0 is the reg used for the func call.
                                        # a0 now holds the address of the 
                                        # figure object

        lw      $s1, 0($a0)             # get a pointer to the figure's vtable
        lw      $s2, 0($s1)             # area function is at a 0 
                                        # offset in the vtable
        
        jalr    $s2                     # use jalr because you know the
                                        # address of the function

        add     $s0, $a0, $zero         # move a0 back to s0
        add     $a0, $v0, $zero         # put v0 into a0 to be printed

        li      $v0, PRINT_INT          #v0 = print a number
        syscall 
...
#
        ...
        lw      $s1, 0($a0)             # get a pointer to the figure's vtable
        lw      $s2, 4($s1)             # perimeter function is at a 4
                                        # offset in the vtable
